...
def f_threshold(img, tipo="Otsu (automático)", valor=127, block=11, C=2):
    ...
    if tipo == "Binario fijo":
        ...
        mask = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, bs, C)
...
def op_suma(img, v=50):   return np.clip(img.astype(np.int32)+int(v), 0, 255).astype(np.uint8)
def op_resta(img, v=50):  return np.clip(img.astype(np.int32)-int(v), 0, 255).astype(np.uint8)
...
def op_log(img, c=40.0):  return np.clip(c*np.log(img.astype(np.float32)+1), 0, 255).astype(np.uint8)
def op_he(img):
    ycc = cv2.cvtColor(img, cv2.COLOR_RGB2YCrCb)
    ycc[:,:,0] = cv2.equalizeHist(ycc[:,:,0])
    return cv2.cvtColor(ycc, cv2.COLOR_YCrCb2RGB)
def op_negativa(img):     return (255 - img.astype(np.int32)).clip(0,255).astype(np.uint8)
...
OPS_PRE = [NINGUNA_PRE, "Suma", "Resta", "Corrección Gamma",
           "Linear Stretching", "Transformada Log", "Ecualización HE", "Negativa", "Escala de Grises"]
...
def plot_edge_comparison(orig, proc, label_proc):
    ...
    datos = [(orig,"Original"),(proc,label_proc),(edges_o,"Bordes Canny — Original"),(edges_p,f"Bordes Canny — {label_proc}")]
...
def plot_hist(imgs, labels):
    ...
    fig.patch.set_facecolor("white")
    colors_rgb = ("#e03131","#2f9e44","#1971c2")
    for ax, img, lbl in zip(axes, imgs, labels):
        ax.set_facecolor("white")
        ax.grid(True, color="#e6e6e6", linewidth=0.8)
        for i,(col,name) in enumerate(zip(colors_rgb,"RGB")):
            ch = min(i, img.shape[2]-1) if img.ndim==3 else 0
            h = cv2.calcHist([img],[ch],None,[256],[0,256]).flatten()
            ax.fill_between(range(256),h,alpha=0.25,color=col)
            ax.plot(h,color=col,lw=1.0,label=name)
        ax.set_title(lbl,color="black",fontsize=9)
        ax.set_xlim(0,255)
        ax.tick_params(colors="black",labelsize=7)
        ax.spines[:].set_color("#cccccc")
        ax.set_xlabel("Intensidad",color="#444444",fontsize=7)
        ax.set_ylabel("Frecuencia",color="#444444",fontsize=7)
        ax.legend(fontsize=7,facecolor="white",labelcolor="black",edgecolor="#cccccc")
    fig.tight_layout(); return fig
...
def plot_perfil(orig, proc, lbl):
    ...
    for ax,(img,title) in zip(axes,[(orig,"Original"),(proc,lbl)]):
        ...
        if img.ndim==3:
            for col,name,ch in zip(("#e03131","#2f9e44","#1971c2"),"RGB",range(3)):
                ax.plot(img[row,:,ch],color=col,lw=0.9,label=name)
        else: ax.plot(img[row,:],color="#1971c2",lw=0.9)
        ax.tick_params(colors="black",labelsize=7); ax.spines[:].set_color("#cccccc")
        ax.legend(fontsize=7,facecolor="white",labelcolor="black",edgecolor="#cccccc")
    axes[-1].set_xlabel("Píxel (columna)",color="black",fontsize=8)
    fig.tight_layout(); return fig
...
def plot_curva(nombre_op, P):
    ...
    ax.grid(True, color="#e6e6e6", linewidth=0.8)
    ax.plot(r,s,color="#2f9e44",lw=2)
    ax.plot([0,255],[0,255],color="#cccccc",lw=0.8,linestyle="--")
    ax.set_title("Curva T(r)",color="black",fontsize=9)
    ax.set_xlabel("r",color="black",fontsize=8); ax.set_ylabel("s",color="black",fontsize=8)
    ax.tick_params(colors="black",labelsize=7); ax.spines[:].set_color("#cccccc")
...
with st.sidebar:
                ...
                if st.button("⬆️ Cargar", key="btn_load", use_container_width=True):
                    ...
                    if datos:
                        ...
                        for k in ["suma_v","resta_v","gamma","ls_min","ls_max","log_c",
                                  ...
                                  "sd","mosh_a","mosh_b","ct","th_tipo","th_val","th_block","th_c",
                                  "an_alpha","an_K","an_niters"]:
                            if k in params_g:
    ...
    with st.expander("💾 Guardar preset actual", expanded=False):
        nombre_preset = st.text_input("Nombre del preset", placeholder="ej: Pipeline_final...", key="preset_nombre")
        if st.button("💾 Guardar", key="btn_save", use_container_width=True):
            if nombre_preset.strip():
                ...
                defaults  = dict(gk=3,gs=1.0,cc=3.0,ct=8,mk=3,bd=7,bsc=50,bss=50,
                                 sk=3,sd="XY",lk=3,ct1=30,ct2=80,uk=3,us=1.5,uf=1.2,
                                 mok_a=1,mosh_a="Cruz",moit_a=1,mok_b=11,mosh_b="Cruz",moit_b=2,
                                 suma_v=50,resta_v=50,gamma=0.4,ls_min=2,ls_max=98,log_c=40.0,
    ...
    with st.expander("📥 Exportar preset activo como archivo", expanded=False):
        nom_exp = st.text_input("Nombre para exportar", key="preset_exp_nombre", placeholder="Pipeline_final...")
        if st.button("📥 Descargar JSON", key="btn_exp_dl", use_container_width=True):
            if nom_exp.strip():
                ...
                defaults2 = dict(gk=3,gs=1.0,cc=3.0,ct=8,mk=3,bd=7,bsc=50,bss=50,
                                 sk=3,sd="XY",lk=3,ct1=30,ct2=80,uk=3,us=1.5,uf=1.2,
                                 mok_a=1,mosh_a="Cruz",moit_a=1,mok_b=11,mosh_b="Cruz",moit_b=2,
                                 suma_v=50,resta_v=50,gamma=0.4,ls_min=2,ls_max=98,log_c=40.0)
                P_exp = {k: st.session_state.get(k, v) for k, v in defaults2.items()}
                data_exp = {"nombre": nom_exp.strip(), "pre_ops": ops_exp, "filt_names": filts_exp, "params": P_exp}
                st.download_button("⬇️ Descargar", data=json.dumps(data_exp, ensure_ascii=False, indent=2).encode(),
                                   file_name=f"{nom_exp.strip().replace(' ','_')}.json",
    ...
    with st.expander("⚡ Pre-procesamiento (Ops. Elementales)", expanded=True):
        ...
        ls_min  = st.slider("Stretch — % mín",      0,10,2,         key="ls_min")
        ls_max  = st.slider("Stretch — % máx",      90,100,98,      key="ls_max")
    ...
    with st.expander("🔵 Filtros Espaciales", expanded=False):
        ...
        us  = st.slider("Unsharp — Sigma",       0.1,5.0,1.5,0.1, key="us")
        uf  = st.slider("Unsharp — Fuerza",      0.1,4.0,1.2,0.1, key="uf")
    ...
    with st.expander("🔴 Morfológico A  (Pasos 1–3)", expanded=True):
        mok_a  = st.slider("Kernel A", 1,15,1,2,  key="mok_a")
    ...
    with st.expander("🟠 Morfológico B  (Pasos 4–5)", expanded=True):
        mok_b  = st.slider("Kernel B", 1,15,11,2, key="mok_b")
        mosh_b = st.selectbox("Forma B",["Rectángulo","Elipse","Cruz"],key="mosh_b")
        moit_b = st.slider("Iters B",  1,5,2,     key="moit_b")
...
with tab1:
    ...
    with col_reset:
        if st.button("🔄 Reset", key="reset_pipeline"):
            for i in range(3):
    ...
    pre_imgs  = [img_orig]; pre_names = ["Original"]; cur_pre = img_orig.copy()
    for nombre in pre_ops:
        if nombre == NINGUNA_PRE: continue
        cur_pre = aplicar_pre(cur_pre.copy(), nombre, P)
        pre_imgs.append(cur_pre.copy()); pre_names.append(nombre)
    img_preprocesada = cur_pre.copy()
    mostrar_tira(pre_imgs, pre_names)
    if len(pre_names) > 1:
        st.markdown(" → ".join([f'<span class="chip-pre">{n}</span>' for n in pre_names]), unsafe_allow_html=True)
    ...
    filt_cols = st.columns(5); filt_names = []
    ...
    filt_imgs = [img_preprocesada]; filt_labels = ["Pre-proc."]; cur_filt = img_preprocesada.copy()
    for i, nombre in enumerate(filt_names):
        if nombre == NINGUNO_FILT: continue
        grupo = "A" if i < 3 else "B"
        cur_filt = aplicar_filtro(cur_filt.copy(), nombre, P, grupo)
        filt_imgs.append(cur_filt.copy()); filt_labels.append(nombre)
    img_final = cur_filt.copy()
    mostrar_tira(filt_imgs, filt_labels)
    if len(filt_labels) > 1:
        st.markdown(" → ".join([f'<span class="chip-filt">{n}</span>' for n in filt_labels]), unsafe_allow_html=True)

    st.markdown("<hr class='stage-sep'>", unsafe_allow_html=True)
    st.markdown('<span class="stage-header stage-res">③ Resultado final</span>', unsafe_allow_html=True)
    col1,col2,col3,col4 = st.columns(4)
    with col1: st.markdown('<p class="img-label">Original</p>',unsafe_allow_html=True); st.image(img_orig,use_container_width=True)
    with col2: st.markdown('<p class="img-label">Pre-procesada</p>',unsafe_allow_html=True); st.image(img_preprocesada,use_container_width=True)
    with col3: st.markdown('<p class="img-label">Final (pipeline completo)</p>',unsafe_allow_html=True); st.image(img_final,use_container_width=True)
    with col4: st.markdown('<p class="img-label">Diferencia (orig → final)</p>',unsafe_allow_html=True); st.image(diferencia(img_orig,img_final),use_container_width=True)

    st.markdown("<hr class='stage-sep'>", unsafe_allow_html=True)
    col_tog1,col_tog2,col_tog3 = st.columns(3)
    with col_tog1: show_hist = st.toggle("📊 Histogramas",        value=False, key="sh")
    with col_tog2: show_metr = st.toggle("📋 Métricas generales", value=False, key="sm")
    with col_tog3: show_edge = st.toggle("🔍 Métricas de bordes", value=False, key="se")

    if show_hist:
        st.markdown("#### Histogramas: Original → Pre-proc → Final")
        fig_h = plot_hist([img_orig,img_preprocesada,img_final],["Original","Pre-procesada","Final"])
    ...
    if show_metr:
        ...
        etapas = [("Pre-procesada",img_preprocesada),("Final (pipeline)",img_final),("Diferencia orig→final",img_diff)]
        tabla = []
        for lbl,img in etapas:
            m = metricas(img_orig,img); m["Etapa"] = lbl; tabla.append(m)
        df = pd.DataFrame(tabla).set_index("Etapa")
        st.dataframe(df.style.background_gradient(cmap="Blues",subset=["SSIM"]).background_gradient(cmap="Reds_r",subset=["MSE"]),use_container_width=True)
        st.markdown("#### Resumen rápido")
        m_final = metricas(img_orig,img_final); m_diff = metricas(img_orig,diferencia(img_orig,img_final))
        col_f1,col_f2,col_f3,col_sep,col_d1,col_d2,col_d3 = st.columns([1,1,1,0.2,1,1,1])
        st.markdown("""<style>
        .mcard{background:#0d1526;border:1px solid #1e2d4a;border-radius:8px;padding:10px;text-align:center;}
        .mcard .mlabel{color:#8899aa;font-size:0.7rem;font-family:'Space Mono',monospace;text-transform:uppercase;letter-spacing:1px;}
        .mcard .mval{font-size:1.3rem;font-weight:700;font-family:'Space Mono',monospace;}
        .mcard .mval.blue{color:#00d4ff;}.mcard .mval.green{color:#51cf66;}.mcard .mval.red{color:#ff6b6b;}
        .mcard .msub{color:#8899aa;font-size:0.65rem;margin-top:2px;}
        </style>""",unsafe_allow_html=True)
        with col_f1: st.markdown(f'<div class="mcard"><div class="mlabel">MSE · Final</div><div class="mval red">{m_final["MSE"]}</div><div class="msub">↓ mejor</div></div>',unsafe_allow_html=True)
        with col_f2: st.markdown(f'<div class="mcard"><div class="mlabel">PSNR · Final</div><div class="mval blue">{m_final["PSNR"]} dB</div><div class="msub">↑ mejor</div></div>',unsafe_allow_html=True)
        with col_f3: st.markdown(f'<div class="mcard"><div class="mlabel">SSIM · Final</div><div class="mval green">{m_final["SSIM"]}</div><div class="msub">↑ mejor · máx 1.0</div></div>',unsafe_allow_html=True)
        with col_d1: st.markdown(f'<div class="mcard"><div class="mlabel">MSE · Diferencia</div><div class="mval red">{m_diff["MSE"]}</div><div class="msub">↓ mejor</div></div>',unsafe_allow_html=True)
        with col_d2: st.markdown(f'<div class="mcard"><div class="mlabel">PSNR · Diferencia</div><div class="mval blue">{m_diff["PSNR"]} dB</div><div class="msub">↑ mejor</div></div>',unsafe_allow_html=True)
        with col_d3: st.markdown(f'<div class="mcard"><div class="mlabel">SSIM · Diferencia</div><div class="mval green">{m_diff["SSIM"]}</div><div class="msub">↑ mejor · máx 1.0</div></div>',unsafe_allow_html=True)
    ...
    if show_edge:
        ...
        for metrica,(val_orig,val_proc) in resultados_borde.items():
            cambio = val_proc-val_orig; signo = "▲" if cambio>0 else "▼"
            filas.append({"Métrica":metrica,"Original":val_orig,"Final":val_proc,"Cambio":f"{signo} {abs(round(cambio,4))}"})
        ...
        st.markdown("""<style>
        .ecard{background:#0d1526;border:1px solid #1e2d4a;border-radius:8px;padding:10px;text-align:center;margin-bottom:6px;}
        .ecard .elabel{color:#8899aa;font-size:0.68rem;font-family:"Space Mono",monospace;text-transform:uppercase;letter-spacing:1px;}
        ...
        .ecard .up{color:#51cf66;}.ecard .down{color:#ff6b6b;}
        </style>""",unsafe_allow_html=True)
        def delta_html(orig,proc,higher_better=True):
            diff=proc-orig; cls="up" if (diff>0)==higher_better else "down"; sign="▲" if diff>0 else "▼"
            return f'<span class="{cls}">{sign} {abs(round(diff,4))}</span>'
        with col_e1: st.markdown(f'<div class="ecard"><div class="elabel">Edge Density</div><div class="eval">{dens_p}%</div><div class="ediff">orig: {dens_o}% &nbsp; {delta_html(dens_o,dens_p)}</div></div>',unsafe_allow_html=True)
        with col_e2: st.markdown(f'<div class="ecard"><div class="elabel">Edge Strength</div><div class="eval">{mes_p}</div><div class="ediff">orig: {mes_o} &nbsp; {delta_html(mes_o,mes_p)}</div></div>',unsafe_allow_html=True)
        with col_e3: st.markdown(f'<div class="ecard"><div class="elabel">Entropía (bits)</div><div class="eval">{ent_p}</div><div class="ediff">orig: {ent_o} &nbsp; {delta_html(ent_o,ent_p)}</div></div>',unsafe_allow_html=True)
        with col_e4: st.markdown(f'<div class="ecard"><div class="elabel">Mask Coverage</div><div class="eval">{cov_p}%</div><div class="ediff">orig: {cov_o}% &nbsp; {delta_html(cov_o,cov_p)}</div></div>',unsafe_allow_html=True)
        ...
        st.caption("**Edge Density** = % píxeles borde (Canny) · **Edge Strength** = magnitud gradiente Sobel · **Entropía** = información Shannon · **Mask Coverage** = % píxeles blancos en máscara binarizada")

    # ── Exportar configuración y métricas ──────────────────────────────────
    st.markdown("<hr class='stage-sep'>", unsafe_allow_html=True)
    st.markdown('<span class="stage-header stage-res">📋 Exportar configuración y métricas</span>', unsafe_allow_html=True)
    ...
    pre_activos  = [st.session_state.get(f"saved_pre_{i}", NINGUNA_PRE) for i in range(3) if st.session_state.get(f"saved_pre_{i}", NINGUNA_PRE) != NINGUNA_PRE]
    filt_activos = [st.session_state.get(f"saved_f{i}", NINGUNO_FILT) for i in range(5) if st.session_state.get(f"saved_f{i}", NINGUNO_FILT) != NINGUNO_FILT]
    ...
    for fn in filt_activos:
        ...
        if fn == "Threshold":     params_usados["Threshold tipo"]     = P["th_tipo"]
        ...
        if fn in F_MORFO:
            ...
            grupo = "A" if pos<3 else "B"; g = grupo.lower()
            params_usados[f"{fn} kernel/forma/iters ({grupo})"] = f"{P[f'mok_{g}']} / {P[f'mosh_{g}']} / {P[f'moit_{g}']}"
    ...
    edge_lines   = "\n".join(f"  {m:<30} orig={v[0]:>8}  final={v[1]:>8}  Δ={round(v[1]-v[0],4):>+9}" for m,v in rb.items())
...
""".strip()
    ...
    st.text_area(label="📋 Configuración y métricas — listo para copiar", value=texto_export, height=370,
                 help="Selecciona todo (Ctrl+A dentro del cuadro) y copia (Ctrl+C)")
    st.download_button(label="⬇️ Descargar como .txt", data=texto_export.encode("utf-8"),
                       file_name=f"pipeline_{nombre_img.rsplit('.',1)[0]}.txt",
                       mime="text/plain", key="dl_config")

with tab2:
    st.subheader("Comparación de filtros individuales — reactivo")
    fuente_cmp = st.radio("Fuente de comparación:", ["Original","Pre-procesada (del pipeline)"], horizontal=True, key="fuente_cmp")
    img_cmp_src = img_orig if fuente_cmp=="Original" else img_preprocesada
    filtros_comp = []
    for i,col in enumerate(st.columns(4)):
        with col: filtros_comp.append(st.selectbox(f"Filtro {chr(65+i)}", FILTROS, key=f"cmp_{i}"))
    resultados = [(n,aplicar_filtro(img_cmp_src.copy(),n,P,"A")) for n in filtros_comp if n!=NINGUNO_FILT]
    if resultados:
        ...
        fig_h = plot_hist([img_cmp_src]+[r[1] for r in resultados],[lbl_src]+[r[0] for r in resultados])
        st.pyplot(fig_h,use_container_width=True); plt.close(fig_h)
    else:
...
with tab3:
    ...
    fuente_graf = st.radio("Imagen de entrada",["Original","Pre-procesada"],horizontal=True,key="fuente_gr")
    ...
    fig_d.tight_layout(); st.pyplot(fig_d,use_container_width=True); plt.close(fig_d)

with tab4:
    st.subheader("Métricas de calidad")
    todas_op = FILTROS[1:] + OPS_PRE[1:]
    sel = st.multiselect("Selecciona filtros/operaciones a comparar", todas_op,
                         default=["Gaussiano","CLAHE","Black-Hat","Corrección Gamma","Ecualización HE"], key="met_sel")
    fuente_met = st.radio("Imagen de referencia",["Original","Pre-procesada"],horizontal=True,key="fuente_met")
    img_met_src = img_orig if fuente_met=="Original" else img_preprocesada
    if sel:
        tabla = []
        for nombre in sel:
            if nombre in FILTROS[1:]: ip = aplicar_filtro(img_met_src.copy(),nombre,P,"A")
            else:                     ip = aplicar_pre(img_met_src.copy(),nombre,P)
            if ip.shape!=img_met_src.shape: ip=cv2.cvtColor(cv2.cvtColor(ip,cv2.COLOR_RGB2GRAY),cv2.COLOR_GRAY2RGB)
            m=metricas(img_met_src,ip); m["Filtro/Op"]=nombre; tabla.append(m)
        ...
        bars=ax.barh(df.index,df["SSIM"],color="#1971c2",alpha=0.85)
        ax.set_xlim(0,1); ax.set_title("SSIM por filtro/op",color="black",fontsize=10)
        ax.grid(True,axis='x',color="#e6e6e6",linewidth=0.8)
        ax.tick_params(colors="black",labelsize=8); ax.spines[:].set_color("#cccccc")
        for bar in bars:
            ax.text(bar.get_width()+0.01,bar.get_y()+bar.get_height()/2,f"{bar.get_width():.4f}",va="center",color="black",fontsize=7)
