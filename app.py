...
def filtro_clahe(img, clip_limit=2.0, tile_size=8):
    # Convierte a LAB para aplicar CLAHE solo al canal de luminosidad
    if img.ndim == 3:
...
with st.sidebar:
    ...
    with st.expander("Filtros Espaciales", expanded=True):
        ...
        clahe_clip = st.slider("CLAHE — Clip Limit",  0.5, 10.0, 2.0, 0.5, key="cc")
        clahe_tile = st.select_slider("CLAHE — Tile Size", options=[4,8,16,32], value=8, key="ct")
...
with tab1:
    ...
    if st.button("▶ Aplicar Pipeline"):
        steps_imgs, steps_names = [img_orig], ["Original"]
        current = img_orig.copy()
        for nombre in pipeline_names:
            if nombre == "— ninguno —": continue
            current = aplicar_filtro(current.copy(), nombre, params)
            steps_imgs.append(current.copy()); steps_names.append(nombre)
        cols_show = st.columns(len(steps_imgs))
        for col, img_s, name_s in zip(cols_show, steps_imgs, steps_names):
            with col: st.image(img_s, caption=name_s, use_container_width=True)
        st.markdown("**Pipeline:** " + " → ".join(
            [f'<span class="step-chip">{n}</span>' for n in steps_names]
        ), unsafe_allow_html=True)
        if len(steps_imgs) > 1:
            st.markdown("---")
            st.markdown("#### Diferencia original vs resultado final")
            st.pyplot(fig_diferencia(img_orig, steps_imgs[-1]), use_container_width=True)
            st.markdown("#### Histogramas")
            st.pyplot(fig_histograma([img_orig, steps_imgs[-1]], ["Original", steps_names[-1]]), use_container_width=True)
...
with tab2:
    ...
    if st.button("⚖️ Comparar Filtros"):
        resultados = [(n, aplicar_filtro(img_orig.copy(), n, params))
